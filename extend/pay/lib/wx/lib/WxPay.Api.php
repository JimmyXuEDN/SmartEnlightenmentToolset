...
class WxPayApi
{
     ...
     *
     * 付款到银行
     * @param WxTransferBank $inputObj
     * @return 成功时返回，其他抛异常
     * @throws WxPayException
     */
    public static function transferToBank($inputObj)
    {
        $url = "https://api.mch.weixin.qq.com/mmpaysptrans/pay_bank";
        //检测必填参数
        if (!$inputObj->IsTrade_noSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数partner_trade_no！");
        }
        if (!$inputObj->IsBank_noSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数bank_no！");
        }
        if (!$inputObj->IsBank_codeSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数bank_code！");
        }
        if (!$inputObj->IsTrue_nameSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数true_name！");
        }
        if (!$inputObj->IsAmountSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数amount！");
        }
        $inputObj->SetMch_id(WxPayConfig::$MCHID);//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
        $inputObj->SetSign();//签名
     ...
     *
     * 付款到钱包
     * @param WxTransferPurse $inputObj
     * @return 成功时返回，其他抛异常
     * @throws WxPayException
     */
    public static function transferToPurse($inputObj)
    {
        ...
        if (!$inputObj->IsOpenidSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数openid！");
        }
        if (!$inputObj->IsBank_codeSet()) {
            throw new WxPayException("付款到银行接口，缺少必填参数bank_code！");
        }
        ...
        $inputObj->SetMch_appid(WxPayConfig::$APPID);//商户号
        $inputObj->SetMch_id(WxPayConfig::$MCHID);//商户号
        $inputObj->SetCheck_name("FORCE_CHECK");
        $inputObj->SetSpbill_create_ip($_SERVER['SERVER_ADDR']);
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
        $inputObj->SetSign();//签名
        $xml = $inputObj->ToXml();
        $response = self::postXmlCurl($xml, $url, true);
