...
class ThirdLogin
{
    ...
    {
        $this->appid = saas_config('wx_official.app_id');
        $this->secret = saas_config('wx_official.app_secret');
    }

    public function get_user_info($scope, $code, $identity_type = 'wechat_official')
    {
        $access_res = cache('wx_access_' . $code);
        if (!$access_res) {
            switch ($identity_type) {
                case 'wechat_web':
                    $app_id = saas_config('wx_official.web_app_id');
                    $secret = saas_config('wx_official.web_app_secret');
                    break;
                default:
                    $app_id = $this->appid;
                    $secret = $this->secret;
